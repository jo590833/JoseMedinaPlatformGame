...
function Coin(pos) {
  this.basePos = this.pos = pos.plus(new Vector(.1, -.5));
  this.size = new Vector(1, 1);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 100;
}
...
//trying out dding another character called points/bullets

var wobbleSpeed = 8, wobbleDist = 0.07;
Points.prototype.act = function(step) {
...
//creating the Points prototype . act
/*
var wobbleSpeed = 12, wobbleDist = .2;
...
Points.prototype.act = function(step, levels){
this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "points" && this.status == null){
    this.actor = this.filter
    this.status = "lost";
    
    this.finishDelay = 2;
  }
 
  else if (type == "coin"/*"points"//unfortunally when i add tis the it wont switch levels*/) {
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        ...
        //in case i cant get the winnerAlert box to work this will be my simple backup
...
window.onload(confirm("WELCOME!! PLEASE HIT OKAY IF YOU READY TO START THE GAME!!!!"));
